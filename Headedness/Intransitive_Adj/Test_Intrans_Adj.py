...
if __name__ == "__main__":
    ...
    # Preprocess Corpus
    glove_list, bert_list = RSA.preprocess_data('./head_adj_corpus.txt')
    ...
    adj = np.array(embed_dict[lexical_idxs[0]])
    ...
    while len(samples) < 100:
        ...
        adj_geom = RSA.calculate_geometry(samp_adj)
        ...
        rsa_adj_dist.append(spearmanr([adj_geom, bert_geom], axis=1)[0])
    ...
    print(f'RSA Adj: {np.mean(rsa_adj_dist)} STD: {np.std(rsa_adj_dist)}')
    ...
    print(f'Sign Test Subj vs. Adj: {sign_test(np.array(rsa_subj_dist) - np.array(rsa_adj_dist))[1]}')
    print(f'Sign Test Verb vs. Adj: {sign_test(np.array(rsa_verb_dist) - np.array(rsa_adj_dist))[1]}\n')
